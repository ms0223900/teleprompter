...
- [x] 編輯區連續輸入後，重新整理頁面，內容與最後編輯一致（與 US-local-02 一併驗收）。
- [x] 切換「編輯／播放」模式不應清空已儲存稿（除非產品另有規格）。
- [x] 效能可接受：長文輸入時介面不卡頓（debounce 生效）。

**驗收說明**：`manuscriptHydrated` 為真後，`text` 變更以 300ms debounce 呼叫 `saveManuscript`；`beforeunload` 與 `visibilitychange`（hidden）立即補寫。模式切換僅改 `mode`，不重置 `text`。
